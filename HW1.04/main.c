...
typedef enum mobs{ //this enum is saying that whenever we sue a mob, it will always be one from this list
    hikers,
    rivals,
    pacers,
    wanderers,
...
//make enum for keeping track of mob information
typedef struct mob{
    mob_type type; //this comes from the previous enum
    pair_t pos;
    int next_move;
    direction_t direction; //well I can't use this otherwise the other directions will be screwed up
...
static int32_t move_cost[num_character_types][num_terrain_types] = {
        { INT_MAX, INT_MAX, 10, 10,      10,      20, 10, INT_MAX, INT_MAX }, //pc
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 15, 10, 15,      15      }, //hiker
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 20, 10, INT_MAX, INT_MAX }, //rival
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 20, 10, INT_MAX, INT_MAX } //other
};

static int32_t move_cost_mobsters[num_mobsters][num_terrain_types] = {
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 15, 10, 15,      15      }, //hiker
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 20, 10, INT_MAX, INT_MAX }, //rival

        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 20, 10, INT_MAX, INT_MAX }, //pacer
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 20, 10, INT_MAX, INT_MAX }, //wanderers
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 20, 10, INT_MAX, INT_MAX }, //stationary
        { INT_MAX, INT_MAX, 10, INT_MAX, INT_MAX, 20, 10, INT_MAX, INT_MAX }, //random walkers
};

static int32_t mobster_cmp(const void *next_move_a, const void *next_move_b)
{
    int diff = ((mob_t *) next_move_a)->next_move - ((mob_t *) next_move_b)->next_move;
//    if (diff != 0)
    return diff;
...
void init_pc()
{
    ...
    heap_insert(&world.cur_map->heap, &world.pc);
}

//This adds all the mafia to the mob_map
void make_Mafia(map_t *m)
{
    int i;
    for (i = 0; i < 10; i++){
        //for temporarily holding the rand values that will be assigned to the current mob's location
        mob_t *new_mobster = malloc(sizeof(mob_t));
        new_mobster->next_move = 0;

        if(i == 0){
            new_mobster->type = pacers;
        }
        else if (i == 1){
            new_mobster->type = rivals;
        }
        else{
            new_mobster->type = rand()% (num_mobsters-1);
        }
        do{
            new_mobster->pos[dim_x] =  rand()% (MAP_X - 2) + 1;
            new_mobster->pos[dim_y] = rand()% (MAP_Y - 2) + 1;

        } while (m->mob_map[new_mobster->pos[dim_y]][new_mobster->pos[dim_x]] != NULL || move_cost_mobsters[new_mobster->type][m->map[new_mobster->pos[dim_y]][new_mobster->pos[dim_x]]] == INT_MAX);
        //putting it into the heap
        heap_insert(&m->heap, new_mobster);
        m->mob_map[new_mobster->pos[dim_y]][new_mobster->pos[dim_x]] = new_mobster;
    }
}
...
static int new_map()
{
    ...
    //to set all the vals inside the mob map to null
    int i;
    int j;
    for(i = 0; i < MAP_X; i++)
    {
        for(j = 0; j < MAP_Y; j++)
        {
            world.cur_map->mob_map[i][j] = NULL;
        }
    }
    heap_init(&world.cur_map->heap, mobster_cmp, NULL);

    make_Mafia(world.cur_map);
    return 0;
}
...
static void print_map()
{
    ...
    for (y = 0; y < MAP_Y; y++) {
        for (x = 0; x < MAP_X; x++) {
            if(world.cur_map->mob_map[y][x] != NULL)
            {
                switch(world.cur_map->mob_map[y][x]->type)
                {
                    case hikers:
                        putchar('h');
                        break;
...
void move_hikers(map_t *m, mob_t *a)
{

}

void move_rivals(map_t *m, mob_t *a)
{

}

void move_pacers(mob_t *a)
{
    int ogY, ogX;
    ogY = a->pos[dim_y];
    ogX = a->pos[dim_x];
    world.cur_map->mob_map[ogY][ogX] = NULL;

    //switch case to change the movement of mob
    switch (a->direction)
    {
        case north:
            //actually move in opposite direction
            a->pos[dim_y]--;
            break;
        case northeast:
            a->pos[dim_y]--;
    ...
    //switch case to change the enum val of direction
    if (move_cost_mobsters[a->type][world.cur_map->map[a->pos[dim_y]][a->pos[dim_x]]] == INT_MAX) {
        //this is for if it can't move any farther and has reached impassable stuff
        switch (a->direction) {
            case north:
                a->direction = south;
                break;
    ...
    if (world.cur_map->mob_map[a->pos[dim_y]][a->pos[dim_x]] != NULL) {
        a->pos[dim_x] = ogX;
        a->pos[dim_y] = ogY;
    }
    //or does it need to look like this
    world.cur_map->mob_map[a->pos[dim_y]][a->pos[dim_x]] = a;
}

//void move_wanderers(map_t *m, mob_t *a) //can't leave their first terrain type
//{
//    int ogY, ogX;
//    ogY = a->pos[dim_y];
//    ogX = a->pos[dim_x];
...
//    if(move_cost_mobsters[a->next_move] != move_cost_mobsters[m[ogY][ogX]]) //aka their cost changes and terrain subsequently changes
//    {
//        //ADD THE SWITCH CASE FOR DIRECTIONS IN HERE
//    }
//
//}

//void move_random_walkers(map_t *m, mob_t *a) //making my code for random_walkers the same as my pacers
//{
//
//}

//the function to determine which move function needs to be called for the specific mob
void move_npcs()
{
    mob_t *a;

    while((a = heap_peek_min(&world.cur_map->heap))->type != pc)
    {
        switch(a->type)
        {
//            case hikers:
...
//                break;
            case pacers:
                move_pacers(a); //switched this back to &a
                break;
//            case wanderers:
//                move_wanderers();
//                break;
...
//                default_reached = 1;
        ...
        heap_remove_min(&world.cur_map->heap);
        a->next_move += move_cost_mobsters[a->type][world.cur_map->map[a->pos[dim_y]][a->pos[dim_x]]];

        heap_insert(&world.cur_map->heap, a);
        //Increment next_move
    }
    //otherwise it is an npc and it will execute the following code
    heap_remove_min(&world.cur_map->heap);
    //Increment next_move
    a->next_move += move_cost_mobsters[a->type][world.cur_map->map[a->pos[dim_y]][a->pos[dim_x]]];
    heap_insert(&world.cur_map->heap, a);
